...
const PhysicsCanvas = () => {
  ...
  useEffect(() => {
    ...
    const socket = io('https://quip-server-7r07.onrender.com'); 
    socketRef.current = socket;
    
    let gameState = { 
      p1: {x: 250, y: 300}, p2: {x: 550, y: 300},
      scores: { p1: 0, p2: 0 }, health: { p1: 100, p2: 100 }, 
      matchState: 'lobby', matchTimer: 180, winner: null, playersReady: 0
    };
    ...
    const spawnParticles = (x, y, color, count, speedConfig) => {
      for (let i = 0; i < count; i++) {
        ...
        particles.push({
          ...
          life: 1.0, decay: Math.random() * 0.03 + 0.02, color, size: Math.random() * 4 + 2
        });
      }
    };
    ...
    socket.on('role', (role) => { 
      myRole = role; 
      setLobbyUI(prev => ({ ...prev, role: role }));
    });
    ...
    socket.on('gameState', (state) => { 
      ...
      if (state.scores.p1 > prevScores.p1) {
        ...
        renderState.p1 = { x: 250, y: 300 }; renderState.p2 = { x: 550, y: 300 };
      }
    ...
    const renderLoop = () => {
      renderState.p1.x += (gameState.p1.x - renderState.p1.x) * 0.3;
      renderState.p1.y += (gameState.p1.y - renderState.p1.y) * 0.3;
      renderState.p2.x += (gameState.p2.x - renderState.p2.x) * 0.3;
      renderState.p2.y += (gameState.p2.y - renderState.p2.y) * 0.3;

      if (impactFrames > 0) {
        const isWhiteBg = impactFrames % 2 === 0;
        ctx.fillStyle = isWhiteBg ? '#ffffff' : '#000000'; ctx.fillRect(0, 0, 800, 600); 
        const fgColor = isWhiteBg ? '#000000' : '#ffffff';
        const midX = (renderState.p1.x + renderState.p2.x) / 2;
        const midY = (renderState.p1.y + renderState.p2.y) / 2;
        
        ctx.save(); ctx.translate(midX, midY); ctx.fillStyle = fgColor;
        for (let i = 0; i < 20; i++) {
          ctx.rotate((Math.PI * 2) / 20); ctx.beginPath();
          ctx.moveTo(40, -5); ctx.lineTo(1000, -10 - Math.random() * 40); ctx.lineTo(1000, 10 + Math.random() * 40); ctx.lineTo(40, 5); ctx.fill();
        }
        ctx.restore();

        ctx.fillStyle = fgColor;
        ctx.beginPath(); ctx.arc(renderState.p1.x, renderState.p1.y, 30, 0, Math.PI * 2); ctx.fill();
        ctx.beginPath(); ctx.arc(renderState.p2.x, renderState.p2.y, 42, 0, Math.PI * 2); ctx.fill();

        impactFrames--; requestAnimationFrame(renderLoop); return;
      }
      ...
      draw3DFighter(renderState.p1.x, renderState.p1.y, 25, '#38bdf8', true);
      draw3DFighter(renderState.p2.x, renderState.p2.y, 35, '#f43f5e', false);
      ...
      if (flashOpacity > 0) {
        ctx.fillStyle = `rgba(255, 255, 255, ${flashOpacity})`; ctx.fillRect(0, 0, 800, 600); flashOpacity -= 0.05; 
      }
      ...
      if (myRole === 'p1') roleText = 'YOU ARE BLUE';
      if (myRole === 'p2') roleText = 'YOU ARE RED';
      if (myRole === 'unassigned') roleText = 'CHOOSE A ROLE';
      ctx.fillText(roleText, 400, 580);
    ...
    const handleEnd = () => {
      if (!isDragging) return;
      isDragging = false; playSound('launch'); 
      const myBall = myRole === 'p1' ? renderState.p1 : renderState.p2;
      ...
      <div style={{ position: 'relative' }}>
        ...
        {lobbyUI.state === 'lobby' && (
          <div style={{ 
            ...
            backgroundColor: 'rgba(2, 6, 23, 0.85)', borderRadius: '12px',
            display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', zIndex: 10 
          }}>
            <h1 style={{ color: '#f8fafc', fontSize: '3.5rem', margin: '0 0 1rem 0', textShadow: '0 0 20px #38bdf8' }}>LOBBY</h1>
            <p style={{ color: '#94a3b8', fontSize: '1.5rem', marginBottom: '3rem' }}>Players Ready: <span style={{ color: lobbyUI.ready === 2 ? '#4ade80' : '#facc15' }}>{lobbyUI.ready} / 2</span></p>
            
            {lobbyUI.role === 'unassigned' && (
              <div style={{ display: 'flex', gap: '2rem' }}>
                <button onClick={() => socketRef.current?.emit('requestRole', 'play')} style={{ padding: '1rem 3rem', fontSize: '1.5rem', backgroundColor: '#38bdf8', color: '#0f172a', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 0 15px #38bdf8' }}>JOIN MATCH</button>
                <button onClick={() => socketRef.current?.emit('requestRole', 'spectate')} style={{ padding: '1rem 3rem', fontSize: '1.5rem', backgroundColor: '#334155', color: '#f8fafc', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold' }}>SPECTATE</button>
              </div>
            )}

            {(lobbyUI.role === 'p1' || lobbyUI.role === 'p2') && (
              <p style={{ color: '#facc15', fontSize: '1.5rem', animation: 'pulse 2s infinite' }}>Waiting for opponent to join...</p>
            )}

            {lobbyUI.role === 'spectator' && (
              <p style={{ color: '#94a3b8', fontSize: '1.5rem' }}>You are spectating. Waiting for fighters...</p>
            )}
          </div>
        )}
      </div>
